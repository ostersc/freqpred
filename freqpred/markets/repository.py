...
async def _upsert_batch(session: AsyncSession, rows: list[dict], now: datetime) -> None:
    ...
    stmt = stmt.on_conflict_do_update(
        ...
        set_={
            ...
            "settlement_value": func.coalesce(stmt.excluded.settlement_value, tbl.c.settlement_value),
            "close_time": stmt.excluded.close_time,
            "yes_bid": stmt.excluded.yes_bid,
            "yes_ask": stmt.excluded.yes_ask,
            "mid_price": stmt.excluded.mid_price,
            "last_price": case(
                (stmt.excluded.last_price > 0, stmt.excluded.last_price),
                else_=tbl.c.last_price,
            ),
            "volume_24h": stmt.excluded.volume_24h,
            "volume_total": stmt.excluded.volume_total,
            "open_interest": stmt.excluded.open_interest,
            "yes_bid_size": stmt.excluded.yes_bid_size,
            "yes_ask_size": stmt.excluded.yes_ask_size,
            "series_ticker": func.coalesce(tbl.c.series_ticker, stmt.excluded.series_ticker),

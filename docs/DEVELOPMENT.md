# freqpred — development and operations

> **This project was retired 2026-08-18.** These are the working procedures as
> they stood at retirement, moved out of the README to keep it readable. They
> are accurate as a record of how the system was operated, but nothing here
> should be run against live markets — `freqpred run` spends real money on
> metered APIs and `--mode live` places real orders. See
> [POSTMORTEM.md](POSTMORTEM.md) for why the project stopped.

Core development commands (tests, lint, migrations, dashboard dev server) live
in the [README](../README.md#development). This file covers the deeper
operational workflows: benchmarking, prompt changes, assessor audits, the
weekly review, and utility scripts.

---

## Contents

- [Benchmarking a model or prompt change](#benchmarking-a-model-or-prompt-change)
- [Changing the signal prompt — the standard workflow](#changing-the-signal-prompt--the-standard-workflow)
- [Auditing the sizing assessor](#auditing-the-sizing-assessor)
- [Weekly profitability review](#weekly-profitability-review)
...
- **Spend**: one call per signal per live arm, priced by the arm's model — ~$0.011 on `z-ai/glm-5.2` (production since 2026-08-09), ~$0.048 on `claude-opus-5`. A two-arm 76-signal frozen-set screen is ~$0.85–$4.50 depending on the arms, and a challenger-only pass against a cached `current` is the cheap common case. Same rule as benchmark runs: check today's spend against the daily cap first (the cap is shared with the live pipeline) and confirm the run with the user.
...
### Adopting a challenger — what to do after a screen wins

Until 2026-08-09 no challenger had ever been adopted through this harness, so the workflow above ended at the verdict and the post-adoption steps were improvised. They are not optional bookkeeping: skipping the first two silently corrupts the *next* screen.

1. **Re-harvest the frozen set's `current` cache to the newly adopted package.** This is the one that bites. `frozen_eval_set.json` stores an already-paid `current`-arm response per signal, and `run_frozen_eval.py` reuses one whenever the **payload** hash matches — but the payload hash does not change when the system prompt or the model changes. A cache harvested under the *previous* package therefore survives adoption intact and will score the retired package as "current" on the next run, with no error. (The v6/opus-4-7 cache did exactly this straight through the v8 adoption.) Repoint it by matching `llm_queries` rows on `query_type='model_eval'`, `success`, `model_used == anthropic.judgment_model`, and `prompt_version LIKE '{_PROMPT_VERSION}%'`, hashing each row's `prompt` and matching it to the stored payload hash — the same rule `scripts/freeze_assessor_eval_set.py` applies. The adoption run's own calls are what you harvest, so this costs nothing and makes the adopted package its own baseline. Verify the payload bytes are unchanged afterwards; only the two cache fields may differ.
2. **Disarm the CHALLENGER block** in `scripts/audit_assessor_enhancement.py` — `CHALLENGER_VERSION`/`CHALLENGER_MODEL`/`CHALLENGER_SYSTEM_PROMPT` back to `None`, `_challenger_payload` back to its `NotImplementedError` — and record the screened package and its result as a comment in that block. Leaving it armed makes the next run measure current-vs-current and bill for a guaranteed null result; `tests/unit/test_audit_harness.py::TestChallengerIsDisarmedAfterAdoption` enforces this and will fail until you do.
3. **Bump `_PROMPT_VERSION` only if the prompt or payload changed.** A model-only swap must *not* bump it — the arms are already distinguishable in `llm_queries` by `model_used`, and bumping would orphan the cache you just re-harvested (the freezer filters on the version prefix).
4. **Re-measure `max_tokens` for a new model** rather than inheriting the incumbent's. Budgets are not transferable: GLM-5.2 returned a `tool_use` block with `input={}` at the value Opus had been audited at, because it spends far more of its budget on reasoning tokens. Check the observed max and p95 output tokens and how many calls hit the cap, then update both `freqpred/metrics/assessment.py` and the pin in `tests/unit/test_signal_assessment.py`.
5. **Update the docs that state what is live**: SPEC.md (§"Signal assessment", the LLM Configuration model list, and the `Last updated` line), the reference-run list below, `config/config.example.yaml`, and the `judgment_model` default in `freqpred/config.py`.
6. **Set a rollout guard** — a date 2–4 weeks out to re-run the methodology on live rows, and the condition that reverts the change.
7. **Transcribe the numbers.** Result CSVs are gitignored, so a run's figures survive only in the reference-run list and SPEC.md.

...
- Follow README → "Auditing the sizing assessor". Validate with the paired two-arm audit in `scripts/audit_assessor_enhancement.py` (`current` = the live production package, needs no setup; `challenger` = the proposed package, defined per experiment in the CHALLENGER block at the top of the script — same signals, live judgment-model calls) and adopt only if the bootstrap CI on the corr(trust_score, outcome) difference clears zero (`scripts/.audit_output/analyze_audit.py`, which also reports the verdict distribution — check the challenger isn't a size_down-only flat tax). Do not adopt on inspection alone — propose the audit run to the user as the required validation step.
